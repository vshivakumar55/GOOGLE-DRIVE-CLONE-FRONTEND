<template>
  <div
    v-if="folderToggle"
    @click="closePopup"
    class="absolute -left-5 -top-20 z-20 flex h-screen w-screen items-center justify-center bg-darkC2/40"
  >
    <div
      @click.stop
      class="w-96 space-y-6 rounded-xl bg-white p-5 shadow-lg shadow-[#bbb]"
    >
      <h2 class="text-2xl">New folder</h2>
      <input
        v-model="folderName"
        class="w-full rounded-md border border-textC py-2 indent-5 outline-textC2"
        type="text"
        placeholder="Untitled folder"
      />
      <div class="flex w-full justify-end space-x-5 pr-3 font-medium text-textC2">
        <button
          type="button"
          @click="closePopup"
          class="rounded-full px-3 py-2 hover:bg-darkC2"
        >
          Cancel
        </button>
        <button
...
export default {
  ...
  methods: {
    closePopup() {
      this.setFolderToggle(false);
    },
    addFolder() {
      this.uploadFolder(this.folderName);
      this.setFolderToggle(false);
    },
  },
};
